...
#window.config(background="red") #to change background
...
def train_classifier():
    data_dir = "/Users/guruteja/Desktop/FaceID Detection/data"
...
def detect_face():
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
        ...
        for (x,y,w,h) in features:
            ...
            if confidence>80:
                if id==1:
                    cv2.putText(img, "Santhosh", (x,y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
                if id==2:
                    cv2.putText(img, "GURUTEJA", (x,y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
                if id==3:
                    cv2.putText(img, "Darvik", (x,y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
                if id==4:
                    cv2.putText(img, "AKSHITH", (x,y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
            else:
